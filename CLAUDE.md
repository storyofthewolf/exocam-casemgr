...
### rpointer reset does not verify the restart files it names (runmgr.py)
`_rpointer_reset_plan` checks that the reference `rpointer.*` files exist in `archive/`(or `long_term/`) and that the destination run dir exists, but **not** that the `.r.`/`.i.` restart `.nc` files those pointers name are actually present in the run dir. A case can pass the preview cleanly, get correct pointers copied in, and still fail at CESM startup because the restart fileset itself is missing or was purged (e.g. by `datamgr.py clean purge-restarts`). The failure surfaces in the run log rather than in runmgr.
...
- **The rpointer reset never checks that the reference `.r.`/`.i.` restart files exist** in the run dir — only that the pointers exist in `archive/`. A pointer naming a missing `.nc` still fails at startup, with a less obvious error. Cheapest real hardening left in this area.
