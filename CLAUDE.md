...
### restart / build.py rebuild --send-it — rpointer.* reset for branch/hybrid cases
...
- `_rpointer_reset_plan()` / `_reset_rpointers()` / `_find_refdir()` (runmgr.py) are the shared implementation, lazily imported by `build.py rebuild`; verified against real `hybrid`/`branch` cases on Discover (preview-only dry run) before landing, for both `restart` and `rebuild`.

## build.py rebuild — recompile an ensemble after a hand-edit

```
build.py rebuild --prefix noO3_grp3 --execute                 # recompile only
build.py rebuild --prefix noO3_grp3 --send-it --execute       # + relaunch
```

Added 2026-07-27 for the case that prompted it: an across-the-ensemble bug fix to hardcoded Fortran (e.g. hand-edited `exoplanet_mod.F90`, or another file under `SourceMods`) where every case in a matrix needs the identical rebuild, with nothing else changing. Owner decision, 2026-07-27: it lives in `build.py`, not `runmgr.py` — **no runmgr verb runs `<case>.build`**, and that test (does it compile?) is what divides the two modules. Submission is not the discriminator, since `make --send-it` already sbatches from `build.py`.
...
**`--send-it` mirrors `make --send-it`: bare `rebuild` only recompiles.** With `--send-it` it also relaunches each successfully compiled case — `CONTINUE_RUN=FALSE`, branch/hybrid rpointer reset, `sbatch`. Those three are one unit and fire only together, so a recompile-only run never leaves a case with reset pointers and no job to consume them. Without `--send-it` a `NOTE` points at `runmgr.py continue/restart` for the launch step.

- **Status gating depends on the mode.** Under `--send-it`, RUNNING/RESUBMITTED are **hard-blocked** (matches `restart`) — it resubmits, so an active case must never be touched. Without `--send-it` the verb is `patch`-shaped and only **flags** them: recompiling a queued case so its next segment picks up the new binary is a supported use.
- Cases with no `<case>.build` are skipped before anything is touched.
- A failed compile is reported per-case and that case is not submitted; the rest of the batch continues (same pattern as `patch`).
...
- The `--send-it` half lazily imports `_rpointer_reset_plan`/`_reset_rpointers`/`_apply_xmlchange`/`_read_case_xml_var` from `runmgr.py`, the same import direction `patch` uses for `_probe_status`. The rpointer logic has one implementation, shared with `restart`.
...
- **Compilation lives in `build.py`.** No `runmgr.py` verb runs `<case>.build` — `generate`/`make` (fresh builds), `patch` (exoplanet_mod.F90 edit + rebuild), and `rebuild` (recompile after a hand-edit) are the only entry points to a compiler. Submission is *not* the dividing line between the two modules, since `make --send-it` and `rebuild --send-it` both sbatch from `build.py` via the shared `submit_case()`; "does it compile?" is.
