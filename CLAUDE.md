...
- `load_case_meta(case, cases_yaml_path)` — reads `cases.yaml`, matches on `meta.case_name`, returns `{'config_type': ..., 'exort_pkg': ...}`. Exits with a clear error if `cases.yaml` is missing or the case is not found (directs user to run `scan.py`). The `cases.yaml` path is `paths.cases_yaml` from `config_registry.yaml` if present, otherwise `cases.yaml` next to the script.
