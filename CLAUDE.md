...
- **`runmgr.py`** — run control tool; `check` subcommand (CaseStatus parsing, SLURM probe, optional hist/energy info; `--energy --keep` retains the atm average as the run-time counterpart to `datamgr avg`); `xml` subcommand (ad-hoc `--query VAR` / `--change VAR=VALUE` over a case set — no CONTINUE_RUN forcing, no sbatch; the only way to inspect/edit XML without launching a run); `submit` subcommand (sbatch a built case as-is, no xmlchange — the launch step after `build.py make`; requires `<case>.run`); `continue` subcommand (set CONTINUE_RUN=TRUE, update STOP_N/RESUBMIT, sbatch); `restart` subcommand (set CONTINUE_RUN=FALSE, apply arbitrary `--set VAR=VALUE` xmlchange calls, sbatch); `rebuild` subcommand (recompile `./<case>.build` — the tool makes no source or XML edits of its own — then CONTINUE_RUN=FALSE + branch/hybrid rpointer reset + sbatch, for relaunching an ensemble after a hand-edit anywhere under `SourceMods`; hard-blocks RUNNING/RESUBMITTED like `restart`, unlike `build.py patch`'s soft-flag, since rebuild also resubmits). Shared helpers `_resolve_cases()` (explicit-names-or-`--prefix`, no `--all`), `_parse_set_pairs()`, `_apply_xmlchange()` (the single `./xmlchange` code path), `_probe_status()` (CaseStatus + SLURM probe), `_rest_stop_warning()` (advisory REST_N > STOP_N check over pending edits merged onto live XML), and `_rpointer_reset_plan()`/`_reset_rpointers()`/`_find_refdir()` (branch/hybrid `run/rpointer.*` reset from the `RUN_REFCASE`/`RUN_REFDATE` reference set, shared by `restart` and `rebuild`) back the run-control subcommands. `rebuild` lazily imports `rebuild_case()` from `build.py` — the same plain `./<case>.build` runner `build.py patch` uses — the one place a compile step crosses from the build-time module into the run-control module.
...
### restart / rebuild — rpointer.* reset for branch/hybrid cases
...
- `_rpointer_reset_plan()` / `_reset_rpointers()` / `_find_refdir()` (runmgr.py) are the shared implementation; verified against real `hybrid`/`branch` cases on Discover (preview-only dry run) before landing, for both `restart` and `rebuild`.

### rebuild — recompile-and-relaunch across an ensemble

Added 2026-07-27 for the case that prompted it: an across-the-ensemble bug fix to hardcoded Fortran (e.g. hand-edited `exoplanet_mod.F90`, or another file under `SourceMods`) where every case in a matrix needs the identical rebuild, with nothing else changing.

**The division of labor with `build.py patch` is scope of the source edit, not whether a compile happens.** `patch` *automates* the edit for one file — `exoplanet_mod.F90`, whose `parameter` constants change often enough to be worth a `--set` interface — then rebuilds, but never resubmits. It is deliberately limited to that file: extending it to arbitrary `SourceMods` source edits would be far more arduous than just editing the file. `rebuild` is the general escape hatch for exactly that — you hand-edit any file anywhere under `SourceMods`, and `rebuild` recompiles and relaunches the ensemble. It makes no source edit of its own, and it is not `restart` (which resubmits and accepts arbitrary `--set` XML edits, but never recompiles). It composes exactly two things: `rebuild_case()` (build.py's plain `./<case>.build` runner, the same one `patch` uses, imported lazily the same way `patch` lazily imports `runmgr._probe_status`) and `restart`'s `CONTINUE_RUN=FALSE` + rpointer-reset + sbatch. No `--set` of any kind — if a parameter or XML value also needs to change, that's `patch` or `restart`'s job, run separately.

- Status gating hard-blocks RUNNING/RESUBMITTED (matches `restart`, not `patch`) — `rebuild` resubmits a job, so an already-active case must never be touched, unlike `patch`'s live-binary-swap allowance.
- Cases with no `<case>.build` are skipped before anything is touched, same rationale as `restart` skipping cases with no `<case>.run`.
- A failed compile is reported per-case and that case is not submitted; the rest of the batch continues (same pattern as `patch`'s per-case build failure handling).
- If a matrix mixes `startup` and `branch`/`hybrid` cases under one `--prefix`, the batch handles both correctly in one pass — `startup` cases just skip the rpointer line.
