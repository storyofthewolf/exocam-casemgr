...
# Retire a case — three tiers:
#   bare retire        write case.yaml tombstone only, then delete everything
#   --keep-*           case.yaml written implicitly + selected artifacts preserved
#   --purge            COMPLETE ERASURE: no case.yaml, no config, nothing written to long-term
#
# --keep-config, --keep-years, --keep-restarts are freely combinable
...
# All retire invocations require --execute and prompt for yes/no confirmation
...
- `generate_clone_script(...)` — same as above but uses `create_clone -clone $CLONE_OF -case $CASE` for Step 1, skips the SourceMods/namelist copy step (Step 2 of newcase), and makes IC file lookup conditional on `ncdata` being explicitly set in the spec. `exoplanet_mod.F90` is also embedded inline via heredoc. When `exort_pkg` ends with `*` (custom RT copied into SourceMods), emits an `xmlchange CAM_CONFIG_OPTS` block in Step 2 to update the inherited `-usr_src` path so it points to the new case's own `SourceMods/src.cam/` directory rather than the clone source's path.
